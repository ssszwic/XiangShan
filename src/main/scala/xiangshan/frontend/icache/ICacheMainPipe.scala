...
class ICacheMainPipe(implicit p: Parameters) extends ICacheModule
{
  ...
  /** Meta request */
  toMeta.valid               := ftq_req_to_meta_valid && !missSwitchBit
  ...
  toPMP.zipWithIndex.map { case (p, i) =>
    p.valid := s2_valid && !missSwitchBit
  ...
  val s2_hit_slot_vec = RegEnable(VecInit((0 until PortNumber).map(port => VecInit((0 until PortNumber).map(getMissSituat(port, _))))), s1_fire)
  val s2_hit_slot = s2_hit_slot_vec.map(_.asUInt.orR)
  ...
  // MSHR error
  (0 until PortNumber).map{ i =>
    when(RegNext(s2_fire && missSlot(i).corrupt)){
